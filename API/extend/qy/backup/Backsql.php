...
class Backsql
{
    ...
    public function __construct($config=[])
    {
        ...
        if(count($config)){
            ...
            $this->config=Config::get('database.connections.mysql');
        $this->config['path'] =  VAE_ROOT.'/data/backup/'; //默认目录
        $this->config['version_path']=ROOT."/backup/";
        $this->config["sqlbakname"] = md5(rand(10000,99999)).".sql";//默认保存文件
        $this->config['version_name']=Config::get('version.qingzhan.version').".sql";
    ...
    private function writeToFile($tables = array(), $ddl = array(), $data = array())
    {
        ...
        $str .= "Search:{$this->config['hostname']}:{$this->config['hostport']}\r\n";
    ...
    * @param array $data
    */
    private function version_writeToFile($tables = array(), $ddl = array(), $data = array())
    {
        $str = "/*\r\nMySQL Database SysBackup Tools\r\n";
        $str .= "Search:{$this->config['hostname']}:{$this->config['hostport']}\r\n";
        $str .= "Database:{$this->config['database']}\r\n";
        ...
        $res = file_put_contents($this->config['version_path'] . $this->config['version_name'], $str);
        $size=$this->getfilesize($this->config['version_name']);
        return $res;
    }
    ...
    public function backup($version="")
    {
        ...
        if (!empty($this->tables)) {
            ...
            if($version=="")
                ...
                $res=$this->version_writeToFile($this->tables, $ddl, $data);
            if ($res) {
                return true;
            } else {
                return false;
            ...
            throw new BaseException(['msg'=>'数据库中没有表!']);
        }
    }
    ...
    public function restore($filename = '')
    {
        ...
        if (!file_exists($path)) {
            throw new BaseException(['msg'=>'SQL文件不存在!']);
        } else {
            $sql = $this->parseSQL($path);
            try {
                $this->handler->exec($sql);
                $data= ['message'=>'还原成功!花费时间'. round(microtime(true) - $this->begin, 2) . 'ms'];
                return $data;
    ...
    {
        if(config('setting.uniacid')==0)
            $ucid=0;
        if(config('setting.uniacid')==1)
            $ucid=TokenService::getCurrentTokenVar('uniacid');
        $database=config('database.connections.mysql.database');
        $path = $this->config['path'] . $filename;
        if (!file_exists($path)) {
            throw new BaseException(['msg'=>'SQL文件不存在!']);
        } else {
            $sql = $this->tmpSQL($path,$ucid,$database);
            try {
                $this->handler->exec($sql);
                echo '安装成功!花费时间', round(microtime(true) - $this->begin, 2) . 'ms';
            } catch (BaseException $e) {
                $this->error = $e->getMessage();
                return false;
            }
     ...
     * @return array|mixed|string
     */
    private function tmpSQL($path = '',$ucid,$database)
    {
        $sql = file_get_contents($path);
        $sql = explode("\r\n", $sql);
        //先消除--注释
        ...
        $sql = implode('', $sql);
        $sql = preg_replace('/\/\*.*\*\//', '', $sql);
        $sql = preg_replace('/ucid/', $ucid, $sql);
        $sql = preg_replace('/database/', $database, $sql);
        $sql = preg_replace('/news_/', $prefix, $sql);
        return $sql;
    }
...
//        $FilePath = scandir($FilePath);
    ...
    {
        $fileName = VAE_ROOT.'/data/backup/'.$file;
        if (file_exists($fileName)) {
            ...
            throw new \Exception("File is abnormal");
        }
    }
}