...
class TokenService
{
    ...
    public static function getCurrentTid()
    {
        $uid = self::getCurrentTokenVar('tid');
        return $uid;
    }
     ...
     * 获取权限值
     */
    public static function getCurrenScope()
    {
    ...
    {
        $rules=self::getCurrentTokenVar('rules');
        if(!$rules)
            throw new TokenException(['msg'=>'token不存在']);
        return $rules;
    }
    ...
    {
        $aid=self::getCurrentTokenVar('job');
        return $aid;
    }

    public static function getCurrentStuId()
    {
        $aid=self::getCurrentTokenVar('stu_id');
        if(!$aid)
            throw new TokenException(['msg'=>'token不存在']);
        return $aid;
    }
