<template>
	<view class="pd24_15 bg-ash" style="background-color: #f4f4f5;"> 
		<view class="order-time-line">
			<view class="order-time-main pt-3">
				<view v-for="(item,index) in datasa" :key="index" class="pb24 mx-3" >
					<order-integralgoodste @upprice="telist()" :datas="item" :xiugai="xiugai"></order-integralgoodste>
				</view>
...
<script>
	...
	import orderIntegralgoodste from '@/components/qy/integralgoodste.vue'
	export default{
			components: {
			        uniPagination,orderIntegralgoodste
			    },
		data(){
			return {
		...
		methods:{
			...
			//用户获取订单
			getList() {
				this.$api.get("order/get_my_order").then(res=>{
					this.datasa=res.data
				})
			},
			//教师获取订单
			telist(){
				this.$http.get(this.$api.teacherCms.get_order).then(res=>{
					this.datasa=res.data
