<template>
	<view>
		<view class="box py-3 px-3">
			<view class="" style="margin-bottom: 20upx;">
				<view class="d-flex justify-content-between w-100">
			...
			<view class="d-flex"  @click="skipinfo(datas.id)">
					<image mode="aspectFill"  :src="datas.course.img.url" v-if="datas.course"
						style="width: 35%; height: auto; margin-right: 5%; border-radius: 5px;" ></image>
				
				<view class="order-list-item-r">
					<view style="width: 300upx;" v-if="datas.course" class="fs-6 fw-bold">
						 {{datas.course.name}}
					</view>
					<view class="mt-2 ft12 cl-notice">价格： {{datas.price}}</view>
					<view class="flex alcenter cl-orange mt-2">
						<text class=" ftw600"> ¥ {{datas.total_price}}</text>
					</view>
				</view>
			</view>
			<view class="d-flex justify-content-end mt-3">
				<!-- <view class="w-25" v-if="datas.pay_statement!=0">
					<u-button  type="warning" @click="open_tui(datas.id)"  size="mini" shape="circle">退 款</u-button>
				</view> -->
				<view class="w-25" v-if="datas.pay_statement ==0">
					<u-button type="error" @click="open_del(datas.id)" size="mini" shape="circle" >删  除</u-button>
				</view>
				<view class="w-25" v-if="datas.pay_statement==0">
					<u-button  type="success" shape="circle" size="mini" @click="skip(1,19)">去支付</u-button>
				</view>
