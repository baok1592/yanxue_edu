	...
	<view class="pay">
		...
		<view class="head">
			...
			<view class="row pricebox pt-8">
				...
				<text class="txt1">{{vip_price}}</text>
			</view>
			...
			<radio-group @change="payChange" class="list-box">
				<block v-for="(item,i) in payData" :key="i">
					<view class="flex-rl list-item">
						<view class="left">
							<view class="iconbox">
								<image class="icon-img" src="@/static/img/weixin.png" mode=""></image>
							</view>
						</view>
...
<script>
	...
	//import diy from "@/common/DIYdemo.js"
	import diy from "@/api/pay.js"
	...
	export default {
		onLoad(option) {
			...
			if (option.price) {
				console.log("sss", option.price)
				this.vip_price = option.price;
			}
			//this.show_data();
		},
		data() {
		...
		methods: {
			...
			submitPay() {
				...
				//创建订单
				// #ifdef MP-WEIXIN
				user.create_vip(token, form).then(res => {
					if (res.status == 200) {
						let data = {
