	...
	<view class="content "   :style="{'background-color': diydata.page_info.page_background_color,'background-image':'url('+img+diydata.page_info.bg_img+')', 'background-size':'100%','background-repeat':diydata.page_info.json}">
		...
		<view id="sort-1" class="sortable sortable-panel" :class="diydata.themeColors">
			<block v-for="(result,index) in diydata.sorts.child" :key="index">
				...
				<st-article-list v-if="result.sorts == 'st-article-list'" :isPhone="diydata.isPhone" pageType="teacher" :dat="result" @getUnique="activeGetUnique" :refresh="diydata.shop_list"></st-article-list>
			...
			<st-shop-list v-if="result.sorts == 'st-shop-list'" :isPhone="diydata.isPhone" project="edu"
				:dat="result" @getUnique="activeGetUnique" :refresh="diydata.shop_list"></st-shop-list>
			...
			get_index(){
				// #ifdef H5
					...
					if (isPhone.indexOf('windows') != -1) {
						const diyRes= diy.getEff(_this,false,"teacherStyle");
						diyRes.then(res=>{
							  res.isPhone = false;
							  _this.diydata=res
						}) 
					} else {
						const diyRes= diy.getEff(_this,true,"teacherStyle");
						diyRes.then(res=>{
							  res.isPhone = true;
							  _this.diydata=res
						}) 
